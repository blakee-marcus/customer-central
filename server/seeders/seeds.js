...
const { Customer, User, Note } = require('../models');
...
db.once('open', async () => {
    ...
    for (let i = 0; i < 50; i += 1) {
        ...
        const createdCustomer = await Customer.create({ name, email, phone, address });
    ...
    for (let i = 0; i < 50; i += 1) {
        ...
        const createdNote = await Note.create({ author: userId, noteBody: noteBody });
